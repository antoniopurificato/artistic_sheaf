...
def evaluate_graph_with_colpali(graph_data, loaded_data):
    ...
    for batch in test_loader:
        with torch.no_grad():
            ...
            clip_image = x_img.squeeze(1) 
            if clip_image.dim() == 3:
                clip_image = clip_image.squeeze(1).squeeze(1)
                
            clip_text = x_text.squeeze(1).to(torch.float32)  
...
def main():
    ...
    loaded_data = load_json(os.path.join(args.base_folder, args.dataset, f"triplets_{args.dataset.lower()}_test.json"))
    ...
    metrics, loader = evaluate_graph_with_colpali(graph_data, loaded_data)
    
    flops = obtain_flops_colpali(model, processor, device, model_type=args.model_type)
