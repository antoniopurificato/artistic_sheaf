...
def build_ekg_multitask(
    image_embeddings: Dict[str, torch.Tensor],        # {task: [N_task, D]} train+test concatenated per task
    gt_labels: Dict[str, Dict[int, int]],             # {task: {local_train_i: class_id}}
    pseudo_labels: Dict[str, Dict[int, int]],         # {task: {local_test_i: class_id}}  (LOCAL indices within that task block)
    num_classes: Dict[str, int],                      # {task: C_task}
    device: torch.device,
    ...
    # --- assign class node ranges + mapping (task,class)->global id
...
def train_gnnboost_multitask(
    gnn: nn.Module,
    data: Data,
    labels_by_task: Dict[str, torch.Tensor],        # {task: [N_task_total]} labels for *samples of that task only*
    num_classes: Dict[str, int],
    train_idx_by_task: Dict[str, torch.Tensor],     # {task: global indices of that task's TRAIN sample nodes}
    epochs: int = 3000,
    lr: float = 1e-3,
    patience: int = 100,
    ...
    for _ in range(epochs):
        ...
        for task, head in heads.items():
            ...
            idx = train_idx_by_task[task]  # global node ids (train samples for that task)

            # labels_by_task[task] are in *task-local* order; we need them aligned with idx.
            # We'll store y_train_task in the same order we created idx (see main fix below).
...
def evaluate_multitask(
    ...
    data: Data,
    y_test_by_task: Dict[str, torch.Tensor],        # {task: [N_test_task]}
    test_idx_by_task: Dict[str, torch.Tensor],      # {task: global indices of that task's TEST sample nodes}
) -> Dict[str, float]:
    gnn.eval()
...
def main(args):
    ...
    # -----------------------
    # Base classifier per task + pseudo-labels
    # -----------------------
    ...
    del base
    # -----------------------
    # Build GT/pseudo dicts (LOCAL indices inside each task's [train,test] concat)
    # -----------------------
    ...
    # -----------------------
    # Build graph (global node space with per-task sample blocks)
    # -----------------------
    ...
    # -----------------------
    # Build per-task global train/test indices + aligned labels
    # -----------------------
    ...
    # -----------------------
    # Train GNNBoost with one head per task
    # -----------------------
    d = next(iter(X_train.values())).size(1)  # embedding dim
    ...
    print("Done.")
    # -----------------------
    # Freeze GNN and get refined embeddings
    # -----------------------
    with torch.no_grad():
        refined = gnn(data).detach()

    # -----------------------
    # Train one linear probe per task on refined embeddings (train nodes only)
    # -----------------------
    print("Training linear probes on refined embeddings...")
    clfs: Dict[str, nn.Module] = {}
    for task in tasks:
        C = int(num_classes[task])
