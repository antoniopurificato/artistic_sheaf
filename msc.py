...
from competitors import evaluate_graph_with_colpali
...
class MSCTripletDataset(Dataset):
    ...
    def __init__(self, json_path: str, base_folder: str = ".", max_captions=30, img_size=224):
        ...
        links = {}
        for d in data:
            img_path = os.path.join(base_folder, d["item1"])
            cap = d["item2"]
...
def evaluate(args):
    ...
    # print(f"\nLoading ground truth from: {args.test_data}")
    # with open(args.test_data) as f:
    #     triplets = json.load(f)
    
    # adj_matrix, img_to_idx, txt_to_idx = make_adj_matrix(triplets)
    # print(f"Adjacency matrix shape: {adj_matrix.shape}", len(img_to_idx), len(txt_to_idx))
    # with torch.no_grad():
    #     for images, caps, idxs in tqdm(test_loader, desc="Processing"):
    #         # Encode images
    #         print(len(caps), 'len captions')
    #         img_embs = encode_images(images, model_img, device)
    #         print(f"Image embeddings shape: {img_embs.shape}")
    #         all_img_embs.append(img_embs.cpu())
    #         print(len(caps), 'len captions')
    #         # Encode captions (average per image)
    #         batch_text_embs = []
    #         for i, cap_group in enumerate(caps):
    #             txt_embs = encode_texts(cap_group, vocab, model_txt, device)
    #             print(f"Text embeddings shape for image {idxs[i]}: {txt_embs.shape}")
    #             batch_text_embs.append(txt_embs.cpu())
                
    #             # Store names for adjacency matrix
    #             for j, cap in enumerate(cap_group):
    #                 link = test_ds.samples[idxs[i]]["links"][j]
    #                 image_names.append(test_ds.samples[idxs[i]]["image"].replace('../SemArt/', '') + link)
    #                 text_names.append(f"{cap}{link}")
            
    #         print(image_names[:5])  # Debugging line to check image names
    #         print(text_names[:5])  # Debugging line to check text names
    #         print(len(image_names), len(text_names))  # Debugging line to check lengths
    #         batch_text_embs = torch.cat(batch_text_embs, dim=0)
    #         all_text_embs.append(batch_text_embs)
    
    # # Concatenate all embeddings
    # all_img_embs = torch.cat(all_img_embs, dim=0)
    # all_text_embs = torch.cat(all_text_embs, dim=0)
    # print(f"Image embeddings shape: {all_img_embs.shape}")
    # print(f"Text embeddings shape: {all_text_embs.shape}")
    # print(f"Total images: {len(image_names)}, Total texts: {len(text_names)}")
    
    # # Compute similarity matrix
    # print("\nComputing similarity matrix...")
     
    # # Convert embeddings to numpy for metric computation
    # img_emb_np = all_img_embs.numpy()
    # txt_emb_np = all_text_embs.numpy()
    
    # sim_matrix_np = get_sim_matrix(
    #     image_names, text_names, 
    #     img_emb_np, txt_emb_np, 
    ...
    # precision_at_k = compute_retrieval_metrics(
    #     torch.tensor(sim_matrix_np), 
    #     torch.tensor(adj_matrix), 
    #     k_values=k_values
    # )
    
    # clip_metrics = compute_clip_metrics(
    #     torch.tensor(all_text_embs), 
    #     torch.tensor(all_img_embs), 
    #     topk=tuple(k_values)
    # )
    
    # accuracy = compute_image_to_text_accuracy(sim_matrix_np, adj_matrix)
    ...
    # print(f"\nImage-to-Text Accuracy: {accuracy:.4f}")
    # print("=" * 60)
