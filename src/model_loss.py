...
class SheafMultimodalGNN(pl.LightningModule):
    def __init__(
        ...
        weights_kl_vs_clip: float = 0.5,
        w_clip_vs_mask: float = 1,
        optimizer:str = "AdamW",
        device: str = 'cpu',
        clip_grad=True,
        out_proj=False,
        ...
        self.convs = nn.ModuleList([
            ...
            device=self._device,
            verbose=verbose
            ) for _ in range(self.sheaf_layers)
        ])
    ...
    def step(self, batch, batch_idx, split='train'):
        ...
        loss = (1 - self.w_clip_vs_mask) * loss_clip + self.w_clip_vs_mask * loss_clip_init 
