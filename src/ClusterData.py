...
def _is_node_aligned_list(v, n):
    return isinstance(v, (list, tuple)) and len(v) == n

def _is_edge_aligned_list(v, e):
    return isinstance(v, (list, tuple)) and len(v) == e
...
class ClusterData(torch.utils.data.Dataset):
    ...
    def _permute_data(self, data: Data, partition: Partition) -> Data:
        ...
        node_perm_list = partition.node_perm.tolist()
        edge_perm_list = partition.edge_perm.tolist()
    
        for key, value in data.items():
            if key == 'edge_index':
                continue
    
            # Tensor node attributes (already handled by PyG utilities):
            if data.is_node_attr(key):
                cat_dim = data.__cat_dim__(key, value)
                out[key] = select(value, partition.node_perm, dim=cat_dim)
    
            # NEW: list/tuple node attributes
            elif _is_node_aligned_list(value, num_nodes):
                T = type(value)
                out[key] = T(value[i] for i in node_perm_list)
    
            # Tensor edge attributes
            elif data.is_edge_attr(key):
                cat_dim = data.__cat_dim__(key, value)
                out[key] = select(value, partition.edge_perm, dim=cat_dim)
    
            # NEW: list/tuple edge attributes
            elif _is_edge_aligned_list(value, num_edges):
                T = type(value)
                out[key] = T(value[i] for i in edge_perm_list)
    ...
    def __getitem__(self, idx: int) -> Data:
        ...
        for key, value in self.data.items():
            ...
            # NEW: list/tuple node attributes
            elif _is_node_aligned_list(value, self.data.num_nodes):
                T = type(value)
                out[key] = T(value[node_start:node_end])
    
            elif self.data.is_edge_attr(key):
                cat_dim = self.data.__cat_dim__(key, value)
                v = narrow(value, cat_dim, edge_start, edge_length)
                if not self.keep_inter_cluster_edges:
                    v = v[edge_mask]
                out[key] = v
    
            # NEW: list/tuple edge attributes
            elif _is_edge_aligned_list(value, edge_start + edge_length):
                T = type(value)
                v = list(value[edge_start:edge_end])
                if not self.keep_inter_cluster_edges:
                    # filter by mask
                    if self.sparse_format == 'csr':
                        v = [v[i] for i, m in enumerate(edge_mask.tolist()) if m]
                    else:
                        v = [v[i] for i, m in enumerate(edge_mask.tolist()) if m]
                out[key] = T(v) if T is tuple else v
...
class ClusterLoader(torch.utils.data.DataLoader):
    ...
    def _collate(self, batch: List[int]) -> Data:
        ...
        for i in range(batch.numel()):
            ...
            indptr = global_indptr[node_start[i]:node_end[i] + 1] - edge_start[i]
        ...
        # Keep edges that connect inside the batch:
        if self.cluster_data.partition.sparse_format == 'csr':
            col, edge_mask = map_index(col, node); row = row[edge_mask]
        else:
            row, edge_mask = map_index(row, node); col = col[edge_mask]
    
        out = copy.copy(self.cluster_data.data)
    
        num_nodes_full = int(self.cluster_data.data.num_nodes)
        num_edges_full = int(global_indptr[-1].item())  # total edges in permuted graph

        for key, value in self.cluster_data.data.items():
            if key == 'num_nodes':
                out.num_nodes = int(cumsum)
                continue
        
            # 1) NODE-ALIGNED LISTS/tuples → extend slices
            if _is_node_aligned_list(value, num_nodes_full):
                T = type(value)
                src = out[key]  # this is the permuted full list
                buf = []
                for s, e in zip(node_start.tolist(), node_end.tolist()):
                    buf.extend(src[int(s):int(e)])
                out[key] = T(buf) if T is tuple else buf
                continue
        
            # 2) NODE-ALIGNED TENSORS → torch.cat
            if self.cluster_data.data.is_node_attr(key):
                cat_dim = self.cluster_data.data.__cat_dim__(key, value)
                out[key] = torch.cat([
                    narrow(out[key], cat_dim, int(s), int(e - s))
                    for s, e in zip(node_start, node_end)
                ], dim=cat_dim)
                continue
        
            # 3) EDGE-ALIGNED LISTS/tuples → flatten per-part slices then mask
            if _is_edge_aligned_list(value, num_edges_full):
                T = type(value)
                src = out[key]
                flat = []
                for s, e in zip(edge_start.tolist(), edge_end.tolist()):
                    flat.extend(src[int(s):int(e)])
                em = edge_mask.tolist()
                flat = [x for x, m in zip(flat, em) if bool(m)]
                out[key] = T(flat) if T is tuple else flat
                continue
        
            # 4) EDGE-ALIGNED TENSORS → cat then `select` by mask
            if self.cluster_data.data.is_edge_attr(key):
                cat_dim = self.cluster_data.data.__cat_dim__(key, value)
                v = torch.cat([
                    narrow(out[key], cat_dim, int(s), int(e - s))
                    for s, e in zip(edge_start, edge_end)
                ], dim=cat_dim)
                out[key] = select(v, edge_mask, dim=cat_dim)
                continue
